...
- **OSM 文件存在**：`原始数据/OSM/mvp_area.osm`
- **DEM 文件存在**：`原始数据/DEM/mvp_area.tif`
...
OSM 原始数据通常是 WGS84 经纬度，不适合直接作为 Houdini / UE5 场景坐标。尤其是国内数据还可能涉及 `GCJ-02`、`BD-09`、`CGCS2000`。
...
- **OSM 数据**：`原始数据/OSM/mvp_area.osm`
- **DEM 数据**：`原始数据/DEM/mvp_area.tif`
- **处理后 GIS 数据**：`原始数据/GIS_Processed/`
