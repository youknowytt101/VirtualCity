...
> 独立道路研究测试管线。这个目录只服务 `ProjectManagement/RoadResearch`，不读写 VirtualCity 主管线的 `RawData/`、`Config/`、`Scripts/`、`Houdini/` 输出。
...
```python
exec(open(r"E:/VirtualCity/ProjectManagement/RoadResearch/road_test_pipeline/scripts/enable_command_port_in_houdini.py", encoding="utf-8").read())
```
...
6. 运行 `build_road_graph.py`，生成 `road_graph.json`。
7. 运行 road graph 自动 QA。
8. 生成 standalone preview 文件。
9. 用 `hcommand` 连接当前 Houdini 的 `18888` 端口。
10. 在当前场景中创建 / 更新道路测试节点并 cook。
...
```powershell
python "ProjectManagement/RoadResearch/road_test_pipeline/scripts/download_overpass.py" --config "ProjectManagement/RoadResearch/road_test_pipeline/config/pattaya_central_500m.area.json"
```
...
```powershell
& "C:/Program Files/Side Effects Software/Houdini 21.0.440/bin/hython.exe" "ProjectManagement/RoadResearch/road_test_pipeline/scripts/houdini_build_road_test.py" --root "ProjectManagement/RoadResearch/road_test_pipeline" --config "ProjectManagement/RoadResearch/road_test_pipeline/config/pattaya_central_500m.area.json"
```
...
这个测试管线已经跑通了独立样本的下载缓存、raw QA、拓扑修复、road graph 构建、standalone preview 和 Houdini 测试 HIP。后续继续往 junction solver 和 lane graph 迭代：

1. raw roads 自动 QA。
2. topology repair。
3. road_graph builder。
4. junction clustering / movement inference。
5. lane_graph builder。
6. Houdini lane-level junction builder。
...
```powershell
python "ProjectManagement/RoadResearch/road_test_pipeline/scripts/analyze_raw_roads.py" --area-id pattaya_central_500m
python "ProjectManagement/RoadResearch/road_test_pipeline/scripts/run_auto_qa.py" --stage raw_roads --area-id pattaya_central_500m
```
...
```powershell
python "ProjectManagement/RoadResearch/road_test_pipeline/scripts/topology_repair.py" --area-id pattaya_central_500m
python "ProjectManagement/RoadResearch/road_test_pipeline/scripts/analyze_raw_roads.py" --area-id pattaya_central_500m --input "ProjectManagement/RoadResearch/road_test_pipeline/data/processed/pattaya_central_500m_roads_repaired.geojson" --output "ProjectManagement/RoadResearch/road_test_pipeline/reports/pattaya_central_500m_repaired_analysis.json"
python "ProjectManagement/RoadResearch/road_test_pipeline/scripts/run_auto_qa.py" --stage topology_repair --area-id pattaya_central_500m
```
...
```powershell
python "ProjectManagement/RoadResearch/road_test_pipeline/scripts/build_road_graph.py" --area-id pattaya_central_500m
python "ProjectManagement/RoadResearch/road_test_pipeline/scripts/run_auto_qa.py" --stage road_graph --area-id pattaya_central_500m
```
...
reports/pattaya_central_500m_road_graph_report.json
reports/qa/pattaya_central_500m_road_graph_qa_report.json
```

直接生成道路预览：

```powershell
python "ProjectManagement/RoadResearch/road_test_pipeline/scripts/generate_road_preview.py" --area-id pattaya_central_500m
```
