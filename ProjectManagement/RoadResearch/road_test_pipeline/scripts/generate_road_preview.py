...
def build_nodes(edges: list[dict[str, Any]]) -> dict[tuple[int, int], dict[str, Any]]:
    ...
    for edge in edges:
        for role in ("a", "b"):
            p = edge[role]
            key = node_key(p)
            item = nodes.setdefault(key, {"points": [], "edge_ends": []})
            item["points"].append(p)
            item["edge_ends"].append({"edge": edge, "role": role})
    for item in nodes.values():
        ...
        item["degree"] = len(item["edge_ends"])
        compute_node_clip_margins(item)
    return nodes


def edge_direction_out(edge_end: dict[str, Any]) -> tuple[float, float]:
    edge = edge_end["edge"]
    pts = edge["points"]
    if edge_end["role"] == "a":
...
def compute_node_clip_margins(node: dict[str, Any]) -> None:
    node["clip_by_edge"] = {}
    if node["degree"] < JUNCTION_MIN_DEGREE:
        return

    ends = []
    for edge_end in node["edge_ends"]:
    ...
    for index, item in enumerate(ends):
        prev_item = ends[index - 1]
        next_item = ends[(index + 1) % len(ends)]
        a0 = angle_between(item["direction"], prev_item["direction"])
        a1 = angle_between(item["direction"], next_item["direction"])
        min_angle = max(min_angle_floor, min(a0, a1))
        neighbor_hw = max(item["half_width"], prev_item["half_width"], next_item["half_width"])
        clip = neighbor_hw / max(0.25, 2.0 * math.sin(min_angle)) + JUNCTION_RADIUS_M
        clip = min(clip, item["edge"]["length_m"] * 0.45, MAX_CLIP_MARGIN_M)
        node["clip_by_edge"][item["edge"]["edge_id"]] = max(0.0, clip)
...
def build_junction_polygon(node: dict[str, Any]) -> list[tuple[float, float]]:
    ...
    for edge_end in sorted(node["edge_ends"], key=lambda item: angle_of(edge_direction_out(item))):
        edge = edge_end["edge"]
        direction = edge_direction_out(edge_end)
        normal = rotate90(direction)
        clip = node.get("clip_by_edge", {}).get(edge["edge_id"], edge["half_width"] + JUNCTION_RADIUS_M)
        center = (pos[0] + direction[0] * clip, pos[1] + direction[1] * clip)
        boundary_points.append((center[0] - normal[0] * edge["half_width"], center[1] - normal[1] * edge["half_width"]))
        boundary_points.append((center[0] + normal[0] * edge["half_width"], center[1] + normal[1] * edge["half_width"]))

    if len(boundary_points) < 3:
        return []
    return sorted(boundary_points, key=lambda p: math.atan2(p[1] - pos[1], p[0] - pos[0]))
...
def build_junction_cluster_polygon(cluster: dict[str, Any]) -> list[tuple[float, float]]:
    ...
    for node in nodes:
        samples.extend(build_junction_polygon(node))
...
def trimmed_road_surface_points(
    ...
    trim_start = start_node.get("clip_by_edge", {}).get(edge["edge_id"], 0.0) if start_node else 0.0
    trim_end = end_node.get("clip_by_edge", {}).get(edge["edge_id"], 0.0) if end_node else 0.0
...
def write_surface_geojson(
        ...
        features.append({
            ...
            "geometry": {
                ...
                "coordinates": [
                    ...
                    for lon, lat in (
                        to_lonlat(point[0], point[1], origin_lon, origin_lat)
    ...
    write_json(path, {
        ...
        "metadata": {
            "area_id": area_id,
            "schema": "road_test_pipeline.roads_preview_centerlines.v1",
            "mode": "centerline_only_no_surfaces",
            "coord_domain": "WGS84",
    ...
    return {
        "road_surface_polygons": 0,
        "road_surface_trimmed_polygons": 0,
        "road_surface_absorbed_by_junction": 0,
        "road_centerline_features": len(edges),
        "junction_patch_polygons": 0,
        "junction_clustered_nodes": 0,
    }
...
def write_obj(
    ...
    for edge in edges:
        ...
        for x, z in edge["points"]:
            vertices.append((x, 0.06, -z))
        lines_out.append(list(range(start, start + len(edge["points"]))))
...
def write_svg(
    ...
    lines = [
        ...
        '<text x="40" y="58" font-family="Arial, Microsoft YaHei" font-size="13" fill="#5b6878">Standalone generation: optimized centerlines only, no road surfaces or junction patches</text>',
    ]

    for edge in edges:
        polyline_points = " ".join(
            f"{x:.2f},{y:.2f}"
            for x, y in (svg_point(point, b, scale, margin) for point in edge["points"])
        )
        lines.append(
            f'<polyline points="{polyline_points}" fill="none" stroke="{centerline_color}" '
            'stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" opacity="0.9"/>'
        )

    counts = Counter(edge["highway"] for edge in edges)
    legend_y = canvas_h - 90
    lines.append(f'<text x="40" y="{legend_y}" font-family="Arial, Microsoft YaHei" font-size="13" fill="#334155">edges: {len(edges)} | junction clusters: {len(junction_clusters)} | classes: {dict(counts)}</text>')
    lines.append("</svg>")
...
def generate(area_id: str, root: Path) -> dict[str, Any]:
    ...
    optimized = processed / f"{area_id}_roads_optimized_centerlines.geojson"
    repaired = processed / f"{area_id}_roads_repaired.geojson"
    raw = processed / f"{area_id}_roads_raw.geojson"
    input_path = optimized if optimized.exists() else repaired if repaired.exists() else raw
    ...
    report = {
        ...
        "input_mode": "optimized_centerlines" if input_path == optimized else "repaired" if input_path == repaired else "raw",
        "outputs": {
            "centerline_geojson": str(surface_geojson),
            "legacy_surface_geojson": str(surface_geojson),
            "obj": str(obj_path),
            "svg": str(svg_path),
        },
        "counts": {
        ...
        "junction_patch_area_m2": {
            "min_m2": 0.0,
            "median_m2": 0.0,
            "max_m2": 0.0,
            "outliers_gt_4x_median": 0,
        },
        "road_surface_extrusion": {
            "enabled": False,
            "generated_surfaces": 0,
            "trimmed_at_junction": 0,
            "absorbed_by_junction": 0,
            "note": "Disabled by centerline-only preview mode.",
        },
        "highway_class_counts": dict(Counter(edge["highway"] for edge in edges).most_common()),
        "notes": [
            "V5 standalone preview keeps optimized centerlines only.",
            "Road surfaces, junction fan polygons and OBJ faces are intentionally disabled.",
        ],
    }
