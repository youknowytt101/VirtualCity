...
# ── 上采样到 UE5 兼容尺寸（505 × 505） ───────────────
# UE5 Landscape 支持: (2^n * k + 1)，如 127, 253, 505, 1009
TARGET = 505

def bilinear(grid, src_cols, src_rows, tx, ty):
    x = tx * (src_cols - 1) / (TARGET - 1)
    y = ty * (src_rows - 1) / (TARGET - 1)
    x0, y0 = int(x), int(y)
    x1 = min(x0 + 1, src_cols - 1)
    y1 = min(y0 + 1, src_rows - 1)
    fx, fy = x - x0, y - y0
    v = (grid[y0][x0] * (1-fx) * (1-fy) +
         grid[y0][x1] * fx * (1-fy) +
         grid[y1][x0] * (1-fx) * fy +
         grid[y1][x1] * fx * fy)
    return v
...
for ty in range(TARGET):
    for tx in range(TARGET):
        h = bilinear(src_grid, cols, r_rows, tx, ty)
...
print(f"  Scale X/Y   : {xy_scale:.1f} cm/px  ≈ {xy_scale/100:.2f} m/px")
