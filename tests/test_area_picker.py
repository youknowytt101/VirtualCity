...
class TestPickerHtml(unittest.TestCase):
    ...
    def test_action_panel_groups_workflow_modules(self):
        ...
        self.assertIn("自然", area_picker._HTML)
        self.assertIn("道路", area_picker._HTML)
        self.assertIn("自然数据处理入口待接入", area_picker._HTML)
        self.assertIn("道路数据处理入口待接入", area_picker._HTML)
        self.assertNotIn("植被数据清洗入口待接入", area_picker._HTML)
        self.assertNotIn("车道数据处理入口待接入", area_picker._HTML)
