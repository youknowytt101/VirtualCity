...
class TestRecookRoadChain(unittest.TestCase):
    ...
    def test_terrain_domain_owns_dem_nodes_and_public_output(self):
        ...
        self.assertIn('net.createNode("normal", "terrain_color")', terrain)
        self.assertIn('final_nodes=("dem_subdivide", "terrain_color")', terrain)

    def test_terrain_and_road_public_outputs_generate_normals(self):
        terrain = self.TERRAIN_PATH.read_text(encoding="utf-8")
        roads = self.ROADS_PATH.read_text(encoding="utf-8")
        self.assertIn('net.createNode("attribwrangle", "terrain_color_cd")', terrain)
        self.assertIn('net.createNode("normal", "terrain_color")', terrain)
        self.assertIn('net.createNode("attribwrangle", "road_surface_color_cd")', roads)
        self.assertIn('net.createNode("normal", "road_surface_color")', roads)
        self.assertIn('parm("type").set(1)', terrain)
        self.assertIn('parm("type").set(1)', roads)
        self.assertIn('parm("normalize").set(1)', terrain)
        self.assertIn('parm("normalize").set(1)', roads)
    ...
    def test_network_layout_groups_domain_nodes_visually_only(self):
        ...
        self.assertIn("NODE_NOTES", layout)
        self.assertIn("apply_node_notes", layout)
        self.assertIn("setComment", layout)
        self.assertIn("DisplayComment", layout)
        self.assertIn("节点备注已写入", layout)
        self.assertIn("[道路] 平滑普通道路硬转角；不处理真实路口。", layout)
        self.assertIn("[旧实验] 早期路口相切圆弧实验节点；不属于当前主道路链路。", layout)
        self.assertIn('"dem_subdivide"', layout)
        self.assertIn('"terrain_color_cd"', layout)
        ...
        self.assertIn('"road_surface_color_cd"', layout)
        self.assertIn('"road_surface_color"', layout)
        self.assertIn("[地形] 为带颜色的最终地形生成顶点法线，作为地形公开输出。", layout)
        self.assertIn("[道路面主输出] 为带颜色的胶囊车道面生成顶点法线；作为 merge_all 的道路输入。", layout)
