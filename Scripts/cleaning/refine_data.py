...
def _write_ready_manifest(staging_dir: Path, area_cfg: dict, manifest: dict,
    ...
    for name in output_names:
        ...
        if not path.exists():
            continue
        fingerprint = dcc.file_fingerprint(path)
        fingerprint["path"] = vc_paths.project_relative(HOUDINI_READY / area_id / name)
        outputs[name] = fingerprint
    ready_manifest = {
        "schema": 1,
        ...
        "qa": {
            "time": qa_report.get("time"),
            "passed": qa_report.get("passed"),
            "summary": qa_report.get("summary", {}),
        },
        "outputs": outputs,
    }
    with open(staging_dir / "ready_manifest.json", "w", encoding="utf-8", newline="\n") as f:
        json.dump(ready_manifest, f, indent=2, ensure_ascii=False)
        f.write("\n")
...
def refine(area_cfg: dict, *, target_level: int = 3, force: bool = False,
    ...
    try:
        ...
        ready_manifest = _write_ready_manifest(staging_dir, area_cfg, manifest, cache_state, report)
        print("  [OK] ready_manifest.json: run_id={} cache={}".format(
            ready_manifest.get("run_id", ""),
            ready_manifest.get("cache_key", ""),
        ))
