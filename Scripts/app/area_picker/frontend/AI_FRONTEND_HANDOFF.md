...
| 编辑器左侧按钮无效, 场景根目录保存失败 | Scene project | `scene_project.js`, then `game_workbench.js` | `saveSceneRoot`, `openSceneRoot`, `VC_GAME_WORKBENCH.saveSceneNow` | `/scene-root`, `/open-scene-root` | `test_editor_scene_actions_have_scene_root_dialog` |
| 底部工程资产目录不显示, 工程资产列表为空, 资产浏览器不刷新, Houdini 白盒同步不到资产目录, 资产拖入场景失败 | Scene assets | `scene_assets.js`, then `game_workbench.js`, then `server.py` | `loadSceneAssets`, `renderAssets`, `syncHoudiniWhiteboxToAssets`, `_scene_assets_status`, `_scene_asset_file_path`, `_sync_houdini_whitebox_to_scene_assets` | `/scene-assets`, `/scene-asset-file`, `/sync-whitebox-to-scene-assets`, `/scene-root` | `test_game_workspace_has_bottom_ui_container` |
