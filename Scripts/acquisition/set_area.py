...
只获取数据并切换 active_area，交给 orchestration/run_pipeline.py 继续编排:
    uv run python Scripts/set_area.py --acquire-only <west> <south> <east> <north> [area_name]
"""
...
if DATA_ONLY and ACQUIRE_ONLY:
    print("[ERR] --data-only 与 --acquire-only 不能同时使用")
    sys.exit(1)
ARGS = [arg for arg in sys.argv[1:] if arg not in ("--data-only", "--acquire-only")]
RUN_SOURCE = os.environ.get("VC_PIPELINE_SOURCE", "set_area")
TILE_IDS = [
    item.strip()
    for item in os.environ.get("VC_PIPELINE_TILE_IDS", "").split(",")
    if item.strip()
]
...
if DATA_ONLY:
    ...
    print("  模式: 仅获取数据（写 active_area，跳过 refine_data 与 Houdini 重算）")
print(f"{'='*50}\n")
...
if TILE_IDS:
    cfg["tile_ids"] = TILE_IDS
RUN_ID = pipeline_state.new_run_id(area_name)
cfg["run_id"] = RUN_ID
pipeline_state.create_run(cfg, source=RUN_SOURCE, run_id=RUN_ID)
if DATA_ONLY:
...
if ACQUIRE_ONLY:
    pipeline_state.update_run(RUN_ID, status="running", phase="raw_data_acquired",
                              message="raw data acquisition and clip cache completed")
    _RUN_FINALIZED = True
    print(f"""
{'='*50}
...
[INFO] 已准备 OSM / DEM / 建筑原始数据，并写入缓存。
       active_area.json 已切换到当前区域。
       下一步由 orchestration/run_pipeline.py 执行 refine_data 与 Houdini recook。
""")
    raise SystemExit(0)
...
refine_result = subprocess.run(
    [sys.executable, str(SCRIPTS / "cleaning" / "refine_data.py"), "--skip-probe"],
    cwd=str(SCRIPTS),
...
recook = subprocess.run(
    ["uv", "run", "python", "-u", "houdini_build/recook_new_area.py"],
    cwd=str(SCRIPTS),
    capture_output=False,
)
